...
namespace XNuvem.Logging
{
    public static class LoggingExtensions
    {
        ...
        private static void FilteredLog(ILogger logger, LogLevel level, Exception exception, string format,
            object[] objects)
        {
            if (logger.IsEnabled(level)) logger.Log(level, exception, format, objects);
        }
    }
}