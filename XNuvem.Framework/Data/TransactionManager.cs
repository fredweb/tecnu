...
namespace XNuvem.Data
{
    public class TransactionManager : ITransactionManager, IDisposable
    {
        ...
        private void EnsureSession(IsolationLevel level)
        {
            ...
            _session = sessionFactory.OpenSession(new XNuvemSessionInterceptor(_interceptors.ToArray(), Logger));
        ...
        #region Session interceptor...

        private class XNuvemSessionInterceptor : IInterceptor
        {
            private readonly ISessionInterceptor[] _interceptors;
            private readonly ILogger _logger;

            public XNuvemSessionInterceptor(ISessionInterceptor[] interceptors, ILogger logger)
            {
                _interceptors = interceptors;
                _logger = logger;
            }
            ...
            bool IInterceptor.OnLoad(object entity, object id, object[] state, string[] propertyNames, IType[] types)
            {
                ...
                return _interceptors.Invoke(i => i.OnLoad(entity, id, state, propertyNames, types), _logger).ToList()
                    .Any(r => r);
            }

            bool IInterceptor.OnFlushDirty(object entity, object id, object[] currentState, object[] previousState,
                string[] propertyNames, IType[] types)
            {
                if (_interceptors.Length == 0) return false;
                return _interceptors
                    .Invoke(i => i.OnFlushDirty(entity, id, currentState, previousState, propertyNames, types), _logger)
                    .ToList().Any(r => r);
            }
            ...
            {
                if (_interceptors.Length == 0) return false;
                return _interceptors.Invoke(i => i.OnSave(entity, id, state, propertyNames, types), _logger).ToList()
                    .Any(r => r);
            }
            ...
            bool? IInterceptor.IsTransient(object entity)
            {
                ...
                return _interceptors.Invoke(i => i.IsTransient(entity), _logger).ToList()
                    .FirstOrDefault(c => c.HasValue && c.Value);
            }

            int[] IInterceptor.FindDirty(object entity, object id, object[] currentState, object[] previousState,
                string[] propertyNames, IType[] types)
            {
                if (_interceptors.Length == 0) return null;
                var retVal = _interceptors
                    .Invoke(i => i.FindDirty(entity, id, currentState, previousState, propertyNames, types), _logger)
                    .Where(r => r != null)
                    .SelectMany(r => r)
                    .ToArray();
            ...
            {
                if (_interceptors.Length == 0) return null;
                return _interceptors.Invoke(i => i.Instantiate(entityName, entityMode, id), _logger)
