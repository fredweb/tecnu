...
namespace XNuvem.FileSystems.AppData
{
    public class AppDataFolder : IAppDataFolder
    {
        ...
        {
            Logger.Information("Storing file \"{0}\" as \"{1}\" in \"App_Data\" folder", sourceFileName,
                destinationPath);
        ...
        private void MakeDestinationFileNameAvailable(string destinationFileName)
        {
            ...
            {
                Logger.Warning("Could not delete recipe execution folder {0} under \"App_Data\" folder",
                    destinationFileName);
                return;
            }
            ...
            {
                if (ex.IsFatal()) throw;
                throw new XNuvemCoreException(
                    string.Format("Unable to make room for file \"{0}\" in \"App_Data\" folder", destinationFileName),
                    ex);
            }
        }
